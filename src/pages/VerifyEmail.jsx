...
const VerifyEmail = () => {
  ...
  const [status, setStatus] = useState("verifying") // verifying, success, error, expired, invalid
  ...
  useEffect(() => {
    ...
    // Only verify once, even if useEffect runs multiple times
    if (token && emailParam && !hasVerified.current) {
    ...
    try {
      ...
      if (response.success) {
        if (response.alreadyVerified) {
          ...
          setStatus("success")
          setMessage(
            "Email verified! Your issuer account is pending admin approval. You will receive an email once approved.",
          )
        } else {
          setStatus("success")
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
              ...
              <div className="space-y-4">
                ...
                {email && (
                  <div className="mt-6 p-4 bg-slate-50 rounded-xl">
                    ...
                    {resendMessage && (
                      <p
                        className={`mt-3 text-sm ${resendMessage.includes("sent") ? "text-green-600" : "text-red-600"}`}
                      >
                        {resendMessage}
                      </p>
                    )}
        ...
        <p className="text-center text-sm text-slate-500 mt-6">
          &copy; {new Date().getFullYear()} XertiQ. Blockchain-Secured Document
          Verification.
        </p>
      </div>
    </div>
